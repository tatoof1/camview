...
const productsHolder = document.getElementById("camanounce")
...
function fetchPageProdcuts (buttonName){   
    ...
    helper.fetchCam(pageNumber,renderProductsCards)   
}

function renderProductsCards(camsArray){
    const cardsHtml = []
    ...
    camsArray.forEach(
        ...
        a2.href= `shop-single2.html?model=${cam.Model}?sensor=${cam.Sensor}?weight=${cam.Weight}?
            releasedate=${cam.Release_Date}?fps=${cam.Max_Fps}?dimensions=${cam.Dimensions}?pictures=${cam.Picutres}??`
